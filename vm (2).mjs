...
export default async (req, context) => {
  ...
  const tbStore = getStore("tiebreaker");
  ...
  if (method === "GET") {
    ...
    if (action === "tiebreaker") {
      try {
        const data = await tbStore.get("entries", { type: "json" });
        return new Response(JSON.stringify(data || []), { headers });
      } catch {
        return new Response("[]", { headers });
      }
  ...
  if (method === "POST") {
    ...
    if (action === "tiebreaker") {
      ...
      if (!name || guess === undefined) {
        return new Response(JSON.stringify({ error: "Mangler navn eller gæt" }), { status: 400, headers });
      }
      let entries = [];
      try {
        entries = await tbStore.get("entries", { type: "json" }) || [];
      } catch {}
      const existing = entries.findIndex(e => e.name === name);
      const entry = { name, guess: Number(guess), submitted: new Date().toISOString() };
      if (existing >= 0) entries[existing] = entry;
      else entries.push(entry);
      await tbStore.set("entries", JSON.stringify(entries));
      return new Response(JSON.stringify({ ok: true }), { headers });
    }
  }
